...
void Stage1::update(void)
{
	if (!_sound)
	{
		SOUNDMANAGER->play("스테이지1배경음");
		SOUNDMANAGER->setVolume("스테이지1배경음", 0.2f);
		_sound = true;
	}
	...
	if (_currentMap == map1)
	{
		if (!_createPlayer)
		{
			...
			if (KEYMANAGER->isOnceKeyDown('E'))
			{
				...
				if (_glassIdx <= 3)
				{
					SOUNDMANAGER->play("유리타격");
					SOUNDMANAGER->setVolume("유리타격", 0.2f);
				}
				else if (_glassIdx == 4)
				{
					SOUNDMANAGER->play("유리파괴");
					SOUNDMANAGER->setVolume("유리파괴", 0.2f);
				}
			}
		...
		for (int i = 0; i < _Fzm.size(); i++)
		{
			...
			if (IntersectRect(&_collider, &PLAYER->_bullet.getPos(), &_Fzm[i]->getPos()))
			{
				...
				if (_Fzm[i]->getDeathSound())
				{
					...
					SOUNDMANAGER->setVolume("사망", 0.2f);
...
void Stage1::openDoorR()
{
	if (PLAYER->getPlayerPos().right >= 1100)
	{
		if (_cutDoorR <= 170)
		{
			...
			if (!_RopenSound)
			{
				SOUNDMANAGER->play("문열림");
				SOUNDMANAGER->setVolume("문열림", 0.2f);
				_RopenSound = true;
			}
		}
		else
		...
		if (_cutDoorR > 0 )
		{
			...
			if (!_RcloseSound)
			{
				SOUNDMANAGER->play("문닫힘");
				SOUNDMANAGER->setVolume("문닫힘", 0.2f);
				_RcloseSound = true;
			}
		}
		else
